...
export default function JLStory() {
  ...
  JL PHOTOS
  ============================================================
  Photos are loaded from the backend.
  Backend endpoint:
  GET /api/jl-photos
  The backend returns image paths like:
  /static/uploads/example.jpg
  This component converts those paths into:
  https://haneulz-backend.onrender.com/static/uploads/example.jpg
  so the Vercel frontend can correctly display the images.
  ============================================================
  */
    ...
    const loadJLPhotos = async () => {
      ...
      try {
        ...
        const response = await fetch(
          `${BACKEND_URL}/api/jl-photos`
        );
        if (!response.ok) {
          throw new Error(
            `Failed to load JL photos: ${response.status}`
          );
        }
  ...
  return (
    <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4">
      {Array.from({
        ...
        <div
          key={index}
          className="overflow-hidden rounded-[1.3rem] border border-[#e3c0ce] bg-white p-2"
        >
          <div className="aspect-square animate-pulse rounded-[1rem] bg-[#f8e1ea]" />
          <div className="px-2 pb-1 pt-3">
            <div className="ml-auto h-2 w-5 animate-pulse rounded bg-[#f3d3df]" />
          </div>
        </div>
      ))}
    </div>
