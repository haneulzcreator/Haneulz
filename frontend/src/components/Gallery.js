...
return (
  ...
  <div className="pointer-events-none absolute inset-0 overflow-hidden">
    ...
    <div className="absolute left-[8%] bottom-32 text-2xl text-[#e5a8c0]">
      ✧
    </div>
  </div>
  {/* =====================================================
  ...
  {!loading && !error && posts.length > 0 && (
    <div className="grid grid-cols-2 gap-4 md:grid-cols-3 md:gap-7">
      {posts.map((post, index) => {
        const originalUrl = post.original_url?.trim();
        return (
          <a
            key={post.id}
            href={originalUrl || undefined}
            target={originalUrl ? "_blank" : undefined}
            rel={originalUrl ? "noreferrer" : undefined}
            onClick={(event) => {
              if (!originalUrl) {
...
return (
  {/* ARCHIVE INTRO */}
  <div className="relative mb-10 overflow-hidden rounded-[2.5rem] border border-[#d9e1f1] bg-[#eef4ff] p-7 shadow-[0_18px_55px_rgba(75,95,135,0.09)] md:p-10">
    <div className="absolute -right-8 -top-8 h-32 w-32 rounded-full bg-[#dce7ff]" />
    ...
    <div className="relative">
      <div className="mb-4 inline-flex items-center gap-2 rounded-full border border-[#d5def0] bg-white/90 px-4 py-2">
        <span className="h-2 w-2 rounded-full bg-[#91a8d7]" />
        <p className="text-[8px] font-bold uppercase tracking-[0.28em] text-[#687795]">
          community archive
        </p>
      </div>
      <div className="flex flex-col gap-8 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <p className="mt-5 max-w-lg text-sm leading-7 text-[#6d7485]">
            A cozy collection of fan art and creative pieces
        ...
        <button
          ...
          className="flex shrink-0 items-center justify-center gap-2 rounded-full bg-[#303542] px-6 py-4 text-[9px] font-bold uppercase tracking-[0.2em] text-white shadow-lg transition hover:-translate-y-1 hover:bg-[#7795cf]"
        >
          <Upload size={13} />
          Add your work
        </button>
  ...
  {!loading && !archiveError && posts.length === 0 && (
    <div className="rounded-[2.5rem] border border-dashed border-[#d9dfeb] bg-white px-6 py-24 text-center shadow-sm">
      ...
      <p className="mx-auto mt-3 max-w-sm text-sm leading-6 text-[#777d8b]">
        The archive is waiting for its first little piece.
      </p>
    </div>
  )}
  {/* POSTS */}
...
/*
  IMPORTANT:
  This background is intentionally SOLID/OPAQUE.
  No transparent cream or glass effect here.
*/
<div
